...
public class PlayerMovement : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (!IsDashing && !IsJumping)
		{
			...
			{
				if(LastOnGroundTime < -0.1f) // Landing
                {
		...
		#region JUMP CHECKS
		if (IsJumping && RB.velocity.y < 0) //Jump Falling
		{
			IsJumping = false;
		...
		if (LastOnGroundTime > 0 && !IsJumping && !IsWallJumping)
        {
			...
			_airJumpsLeft = Data.airJumps; //Double Jump
		...
		if (!IsDashing)
		{
			...
			//Double Jump
			else if (LastPressedJumpTime > 0 && _airJumpsLeft > 0 && !_isFalling)
			{
				IsJumping = true;
				IsWallJumping = false;
				_isJumpCut = false;
	...
	private bool CanDoubleJump() //Double Jump
    {
		return IsJumping;
    }
